...
- Proxmox CT 103 · `192.168.1.100:9091` · VLAN 20 Servers
...
UCG-Fiber DHCP (VLAN 10)
    Option 66 = 192.168.1.100 (NovaSCM TFTP)
    Option 67 = ipxe.efi
    │
    ▼
TFTP :69 → NovaSCM serve ipxe.efi
    │
    ▼
iPXE → GET http://192.168.1.100:9091/api/boot/{mac}
...
| NovaSCM (CT 103) | 192.168.1.100 | 20 | Server API :9091, TFTP :69 |
| Windows Server AD | 192.168.1.10 | 20 | DC polariscore.it |
| Pi-hole | 192.168.1.253 | 20 | DNS |
| PC client | 192.168.10.x | 10 | Target deploy |

**Dominio:** `polariscore.it` (NON `.local`)
**DHCP Option 66/67:** da configurare su VLAN 10, next-server=192.168.1.100, filename=ipxe.efi
...
### M-3 · `deploy/autounattend.xml` credenziali hardcoded (`CHANGE_ME`)
...
Aggiornato: server default → `192.168.1.100:9091`, versione → `2.2.1`
