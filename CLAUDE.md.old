...
│ (C#, Windows)       │                     │ CT 103 — 192.168.1.100  │
...
| `deploy/pxe.conf` | Config proxyDHCP backup per dnsmasq (NovaSCM iPXE @ 192.168.1.100) |
...
| Risorsa | IP | VLAN |
|---|---|---|
| NovaSCM server (CT 103) | 192.168.1.100 | 20 (Servers) |
| PC client | 192.168.10.x | 10 (Trusted) |
| UCG-Fiber (gateway/DHCP) | 192.168.10.1 / .20.1 | tutte |
| Pi-hole DNS | 192.168.1.253 | 20 |
| Windows Server AD | 192.168.1.10 | 20 |
| Dominio | polariscore.it | — |

DHCP option 66/67 vanno sulla **VLAN 10** (dove sono i client), puntano a 192.168.1.100.
...
scp root@192.168.1.122:/DATA/win11-extracted/boot/bcd      /opt/novascm/dist/winpe/BCD
scp root@192.168.1.122:/DATA/win11-extracted/boot/boot.sdi /opt/novascm/dist/winpe/boot.sdi
scp root@192.168.1.122:/DATA/win11-extracted/sources/boot.wim /opt/novascm/dist/winpe/boot.wim
...
#    VLAN 10: next-server = 192.168.1.100, filename = ipxe.efi
```
