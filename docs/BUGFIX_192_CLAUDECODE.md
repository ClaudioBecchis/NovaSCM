...
_PXE_SETTINGS_DEFAULTS = {
    "pxe_iventoy_ip":   "192.168.1.100",   # ← stesso IP del server NovaSCM
    "pxe_iventoy_port": "10809",
    ...
}
```

Il default `192.168.1.100` è l'IP del server NovaSCM stesso. Se iVentoy gira su un host separato (scenario comune), il boot iPXE andrà su `chain http://192.168.1.100:10809/boot.ipxe` e fallirà silenziosamente — iPXE cadrà su `local_boot` senza errore esplicito, rendendo difficile il debug.
