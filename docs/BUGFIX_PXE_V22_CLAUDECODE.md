...
```bash
curl http://192.168.1.100:9091/api/autounattend/QUALSIASI-PC
...
```
# Riga 1: # ProxyDHCP — iVentoy @ 192.168.1.122
# Riga 7: pxe-service=X86-64_EFI,"Network Boot (UEFI)",snponly.efi,192.168.1.122
# Riga 8: pxe-service=BC_EFI,"Network Boot (EFI-BC)",snponly.efi,192.168.1.122
```
...
log-dhcp
pxe-service=X86-64_EFI,"NovaSCM PXE Boot (UEFI)",ipxe.efi,192.168.1.100
pxe-service=BC_EFI,"NovaSCM PXE Boot (EFI-BC)",ipxe.efi,192.168.1.100
```
...
**Righe:** 25-26 (password WDS in chiaro), 144-158 (password base64), 165 (IP `192.168.1.122` obsoleto)

```xml
<Password>CHANGE_ME</Password>     <!-- riga 26 — plaintext -->
```

Riga 165 punta a `192.168.1.122` (vecchio CT 110 iVentoy).
...
```
Network: 192.168.1.100:9091
```

Se l'IP di CT 103 è cambiato a `192.168.1.100`, aggiornare. Se è ancora `.110`, il documento PXE_INTEGRATION_CLAUDECODE.md va corretto (usa `.103` in 15+ punti).
