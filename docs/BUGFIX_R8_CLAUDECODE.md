...
```json
{
  ...
  "url": "http://192.168.1.100:9091/api/download/NovaSCM.exe",
  ...
}
```

L'IP `192.168.1.100` è l'indirizzo interno del server NovaSCM nel homelab (VLAN 20 Servers). Committato in chiaro in un repository pubblico GitHub. Chiunque legga il repo conosce la topologia di rete interna.
