...
DHCP (UCG-Fiber, VLAN 10 Trusted)
    next-server = 192.168.1.100   ← CT 103 (NovaSCM)
    filename    = "ipxe.efi"
    │
    ▼
TFTP → NovaSCM (CT 103):69/udp
    ...
    ▼
iPXE chainload HTTP
    GET http://192.168.1.100:9091/api/boot/{mac}
...
iPXE carica wimboot + WinPE via HTTP da NovaSCM
    kernel  http://192.168.1.100:9091/api/pxe/file/wimboot
    initrd  http://192.168.1.100:9091/api/pxe/file/BCD          BCD
    initrd  http://192.168.1.100:9091/api/pxe/file/boot.sdi     boot.sdi
    initrd  http://192.168.1.100:9091/api/pxe/file/boot.wim     boot.wim
    initrd  http://192.168.1.100:9091/api/autounattend/{pc_name} autounattend.xml
    boot
    │
    ▼
...
_PXE_SETTINGS_DEFAULTS = {
    ...
    "pxe_default_dc_ip":      "192.168.1.10",
...
La VLAN 10 deve poter raggiungere CT 103 (192.168.1.100) sulle porte:
...
#!ipxe
dhcp
chain http://192.168.1.100:9091/api/boot/${net0/mac} || sanboot --no-describe --drive 0x80
EOF
...
tftp 192.168.1.100 -c get ipxe.efi /tmp/test.efi && ls -lh /tmp/test.efi

# 2. Simula boot iPXE (curl)
curl http://192.168.1.100:9091/api/boot/AA:BB:CC:DD:EE:FF
# Atteso: script iPXE con kernel wimboot + initrd BCD/boot.sdi/boot.wim

# 3. Verifica file WinPE serviti
curl -I http://192.168.1.100:9091/api/pxe/file/wimboot
# Atteso: 200 OK
curl -I http://192.168.1.100:9091/api/pxe/file/boot.wim
# Atteso: 200 OK (Content-Length ~300-500MB)

# 4. Verifica autounattend.xml dinamico
# (prima creare un CR di test con pc_name=TEST-PC)
curl http://192.168.1.100:9091/api/autounattend/TEST-PC
...
curl http://192.168.1.100:9091/api/pxe/hosts \
  -H "X-Api-Key: TUA_CHIAVE" | python3 -m json.tool

# 7. Verifica health check PXE completo
curl http://192.168.1.100:9091/api/pxe/status \
  -H "X-Api-Key: TUA_CHIAVE" | python3 -m json.tool
...
| **Config** | Tutte | IP NovaSCM aggiornato a 192.168.1.100 (CT 103) |
...
scp root@192.168.1.122:/DATA/win11-extracted/sources/install.wim \
...
wpeinit
certutil -urlcache -split -f http://192.168.1.100:9091/api/pxe/file/install.wim X:\sources\install.wim
```
...
```
**Nota:** l'endpoint `/api/boot/{mac}` deve essere raggiungibile dall'indirizzo IP in `ipxe.efi`. Usare un `ipxe.efi` compilato con script embed che punta a `http://192.168.1.100:9091/api/boot/`.
