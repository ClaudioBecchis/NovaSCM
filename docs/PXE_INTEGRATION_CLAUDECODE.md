...
PC acceso
    ...
    ▼
DHCP (UCG-Fiber)
    next-server = 192.168.1.100   ← NovaSCM
    filename    = "ipxe.efi"
    │
    ▼
TFTP → NovaSCM:69/udp
...
def pxe_boot_script(mac: str):
    ...
    iventoy_ip = pxe_cfg.get("iventoy_ip", "192.168.1.100")
  ...
  <div class="card card-compact" style="margin-bottom:16px">
    <div class="card-grid-4">
      <div class="field-group">
        ...
        <input type="text" id="pxe-iventoy-ip" placeholder="192.168.1.100">
      </div>
      <div class="field-group">
...
curl http://192.168.1.100:9091/api/cr \
  -H "X-Api-Key: TUA_CHIAVE" | python3 -m json.tool
```
